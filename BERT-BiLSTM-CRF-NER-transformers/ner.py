...
def evaluate(args, data, model, id2label, all_ori_tokens):
    ...
    for b_i, (input_ids, input_mask, segment_ids, label_ids) in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        with torch.no_grad():
            if n_gpu > 1:
                logits = model.module.predict(input_ids, segment_ids, input_mask)
            else:
...
def main():
    ...
    parser.add_argument("--canshu", default=None, type=str)
    parser.add_argument("--label_lu", default=None, type=str)
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.do_eval:
                ...
                if f1_score > best_f1:
                    ...
                    torch.save(args, os.path.join(args.output_dir, args.canshu))
    ...
    if args.do_test:
        ...
        args = torch.load(os.path.join(args.output_dir, args.canshu))
        ...
        with open(os.path.join(args.output_dir, args.label_lu), "w", encoding="utf-8") as f:
